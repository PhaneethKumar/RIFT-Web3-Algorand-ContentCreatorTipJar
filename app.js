...
function checkAlgorandSDK() {
  if (
    typeof algosdk !== "undefined" &&
    (typeof PeraWalletConnect !== "undefined" ||
      typeof WalletConnect !== "undefined")
  ) {
...
async function connectWallet() {
  if (!useRealAlgorand) {
    const mockWallet =
      localStorage.getItem("mockWallet") || generateMockWallet();
...
function showWalletModal() {
  return new Promise((resolve) => {
    ...
    modal.querySelector("#peraBtn").onclick = () => {
      document.body.removeChild(modal);
      resolve("pera");
    };
    modal.querySelector("#wcBtn").onclick = () => {
      document.body.removeChild(modal);
...
function updateWalletUI() {
  ...
  // Update wallet address field on profile creation page if it exists
...
async function sendTip(recipientAddress, amount) {
  ...
  try {
    if (useRealAlgorand && walletType !== "mock") {
      ...
      const txId = await window.AlgorandSDK.signAndSendTransaction(
        txn,
        walletType,
      );
